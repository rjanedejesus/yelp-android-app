package com.yep.android.features.base;
...
public class BasePresenter<T extends MvpView> implements Presenter<T> {
    ...
    /**
     * Check if the session is still valid. Api returns Invalid token if not.
     * In that case, the app is automatically logged out. Login screen should be shown.
     * @param dataManager
     * @param status
     * @param message
     * @return
     */
    public boolean isValidSession(DataManager dataManager, String status, String message) {
        if(status.equalsIgnoreCase("error") && message.equalsIgnoreCase("Invalid Token")) {
            logout(dataManager, true);
            return false;
        }
        return true;
    }
    ...
    /**
     * On logout, remove user cache and go back to login screen.
     * @param dataManager
     */
    public void logout(DataManager dataManager, boolean sessionLogout) {
        dataManager.setCurrentUser(null);
//        dataManager.setToken(null);

        if(sessionLogout)
            getView().logoutSession();
        else getView().logout();
    }
